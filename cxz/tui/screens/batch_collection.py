...
class EditRecordModal(Screen):
        ...
        yield Vertical(
            ...
            Horizontal(
                Vertical(
                    ...
                    Select(conditions, value=self.record["condition"], id="condition-select"),
                ),
                Vertical(
                    Static("Sleeve Condition:"),
                    Select(conditions, value=self.record["sleeve_condition"], id="sleeve-condition-select"),
                ),
            ),
            Static("Notes:"),
    ...
    async def _update_worker(self, condition: str, sleeve_condition: str, notes: str) -> Dict[str, Any]:
    ...
    def on_worker_state_changed(self, event: Worker.StateChanged) -> None:
        ...
        if event.worker.name == "_update_worker" and event.state == WorkerState.SUCCESS:
            ...
            if result["success"]:
                ...
                self.app.notify(f"❌ Failed to update record: {result.get('error', 'Unknown error')}")
    ...
    def on_worker_state_changed(self, event: Worker.StateChanged) -> None:
        ...
        if event.worker.name == "_remove_worker" and event.state == WorkerState.SUCCESS:
            ...
            if result["success"]:
                ...
                self.app.notify(f"❌ Failed to remove record: {result.get('error', 'Unknown error')}")
...
class BatchCollectionScreen(Screen):
    ...
    async def _load_records_worker(self) -> List[Dict[str, Any]]:
        ...
        try:
            records = await self.db_service.get_batch_records(include_added_to_discogs=True)
    ...
    def on_worker_state_changed(self, event: Worker.StateChanged) -> None:
        ...
        if worker_name == "_load_records_worker" and event.state == WorkerState.SUCCESS:
            result = event.worker.result
            if "error" in result:
                self.query_one("#batch-status", Static).update(f"❌ Error loading records: {result['error']}")
            else:
            ...
            if result["success"]:
                ...
                self.app.notify(f"❌ Failed to clear records: {result.get('error', 'Unknown error')}")

    def display_batch_records(self, records: List[Dict[str, Any]], stats: Dict[str, int]) -> None:
        """Display batch records in the table."""
        status_widget = self.query_one("#batch-status", Static)
        table = self.query_one("#batch-table", BatchCollectionTable)

        if not records:
            status_widget.update("📭 No records in batch collection. Use Search to add records.")
            return
        ...
        for i, record in enumerate(records):
            ...
            table.add_row(title, year, condition, sleeve_condition, notes, status, key=str(i))
    ...
    def action_publish_to_discogs(self) -> None:
        ...
        self.query_one("#batch-status", Static).update(f"🚀 Publishing {len(pending_records)} records to Discogs...")
    ...
    def handle_publish_result(self, result: Dict[str, Any]) -> None:
        ...
        if success_count == total_count:
            self.app.notify(f"🎉 Successfully published all {success_count} records to Discogs!")
        elif success_count > 0:
            self.app.notify(f"✅ Published {success_count}/{total_count} records. {failed_count} failed.")
        else:
            self.app.notify(f"❌ Failed to publish any records. Check your Discogs credentials.")
