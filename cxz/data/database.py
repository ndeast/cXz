...
class DatabaseService:
    ...
    async def get_batch_records(
        self, include_added_to_discogs: bool = False
    ) -> List[Dict[str, Any]]:
    ...
    async def remove_batch_record(self, record_id: int) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "DELETE FROM batch_records WHERE id = ?", (record_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
    ...
    async def get_record_by_discogs_id(
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT * FROM batch_records WHERE discogs_id = ?", (discogs_id,)
            )
    ...
    async def clear_added_records(self) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "DELETE FROM batch_records WHERE added_to_discogs = TRUE"
            )
            conn.commit()
            return cursor.rowcount
    ...
    async def get_batch_stats(self) -> Dict[str, int]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            row = cursor.fetchone()
            return {"total": row[0], "pending": row[1], "added": row[2]}
